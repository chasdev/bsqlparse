...
class TestFormat(object):
    def test_keywordcase(self):
        ...
        res = bsqlparse.format(sql, keyword_case='upper')
        assert res == 'SELECT * FROM bar; -- select foo\n'
        res = bsqlparse.format(sql, keyword_case='capitalize')
        assert res == 'Select * From bar; -- select foo\n'
        res = bsqlparse.format(sql.upper(), keyword_case='lower')
        assert res == 'select * from BAR; -- SELECT FOO\n'

    def test_keywordcase_invalid_option(self):
        sql = 'select * from bar; -- select foo\n'
        with pytest.raises(bsqlparseError):
            bsqlparse.format(sql, keyword_case='foo')

    def test_identifiercase(self):
        sql = 'select * from bar; -- select foo\n'
        res = bsqlparse.format(sql, identifier_case='upper')
        assert res == 'select * from BAR; -- select foo\n'
        res = bsqlparse.format(sql, identifier_case='capitalize')
        assert res == 'select * from Bar; -- select foo\n'
        res = bsqlparse.format(sql.upper(), identifier_case='lower')
        assert res == 'SELECT * FROM bar; -- SELECT FOO\n'

    def test_identifiercase_invalid_option(self):
        sql = 'select * from bar; -- select foo\n'
        with pytest.raises(bsqlparseError):
            bsqlparse.format(sql, identifier_case='foo')
    ...
    def test_strip_comments_single(self):
        ...
        res = bsqlparse.format(sql, strip_comments=True)
    ...
    def test_strip_comments_invalid_option(self):
        sql = 'select-- foo\nfrom -- bar\nwhere'
        with pytest.raises(bsqlparseError):
            bsqlparse.format(sql, strip_comments=None)
    ...
    def test_strip_ws(self):
        f = lambda sql: bsqlparse.format(sql, strip_whitespace=True)
        s = 'select\n* from      foo\n\twhere  ( 1 = 2 )\n'
    ...
    def test_strip_ws_invalid_option(self):
        s = 'select -- foo\nfrom    bar\n'
        with pytest.raises(bsqlparseError):
            bsqlparse.format(s, strip_whitespace=None)
...
class TestFormatReindentAligned(object):
    @staticmethod
    def formatter(sql):
        return bsqlparse.format(sql, reindent_aligned=True)
...
class TestSpacesAroundOperators(object):
    @staticmethod
    def formatter(sql):
        return bsqlparse.format(sql, use_space_around_operators=True)
...
class TestFormatReindent(object):
    def test_option(self):
        with pytest.raises(bsqlparseError):
            bsqlparse.format('foo', reindent=2)
        with pytest.raises(bsqlparseError):
            bsqlparse.format('foo', indent_tabs=2)
        with pytest.raises(bsqlparseError):
            bsqlparse.format('foo', reindent=True, indent_width='foo')
        with pytest.raises(bsqlparseError):
            bsqlparse.format('foo', reindent=True, indent_width=-12)
        with pytest.raises(bsqlparseError):
            bsqlparse.format('foo', reindent=True, wrap_after='foo')
        with pytest.raises(bsqlparseError):
            bsqlparse.format('foo', reindent=True, wrap_after=-12)
        with pytest.raises(bsqlparseError):
            bsqlparse.format('foo', reindent=True, comma_first='foo')

    def test_stmts(self):
        f = lambda sql: bsqlparse.format(sql, reindent=True)
    ...
    def test_identifier_list_with_wrap_after(self):
        f = lambda sql: bsqlparse.format(sql, reindent=True, wrap_after=14)
        s = 'select foo, bar, baz from table1, table2 where 1 = 2'
        assert f(s) == '\n'.join([
            'select foo, bar,',
            '       baz',
    ...
    def test_identifier_list_comment_first(self):
        f = lambda sql: bsqlparse.format(sql, reindent=True, comma_first=True)
    ...
    def test_duplicate_linebreaks(self):
        ...
        assert f(s) == '\n'.join([
            ...
            'from foo'])
        s = 'select c1 -- column1\nfrom foo'
        r = bsqlparse.format(s, reindent=True, strip_comments=True)
        assert r == '\n'.join([
            'select c1',
            'from foo'])
        s = 'select c1\nfrom foo\norder by c1'
...
class TestOutputFormat(object):
    def test_python(self):
        sql = 'select * from foo;'
        f = lambda sql: bsqlparse.format(sql, output_format='python')
    ...
    def test_php(self):
        sql = 'select * from foo;'
        f = lambda sql: bsqlparse.format(sql, output_format='php')
        assert f(sql) == '$sql = "select * from foo;";'
        f = lambda sql: bsqlparse.format(sql, output_format='php',
                                        reindent=True)
        assert f(sql) == '\n'.join([
    ...
    def test_sql(self):
        ...
        sql = 'select * from foo;'
        f = lambda sql: bsqlparse.format(sql, output_format='sql')
        assert f(sql) == 'select * from foo;'

    def test_invalid_option(self):
        sql = 'select * from foo;'
        with pytest.raises(bsqlparseError):
            bsqlparse.format(sql, output_format='foo')
...
def test_truncate_strings():
    ...
    formatted = bsqlparse.format(sql, truncate_strings=10)
    assert formatted == "update foo set value = 'xxxxxxxxxx[...]';"
    formatted = bsqlparse.format(sql, truncate_strings=3, truncate_char='YYY')
...
def test_truncate_strings_invalid_option2(option):
    with pytest.raises(bsqlparseError):
        bsqlparse.format('foo', truncate_strings=option)
...
def test_truncate_strings_doesnt_truncate_identifiers(sql):
    formatted = bsqlparse.format(sql, truncate_strings=2)
...
def test_format_right_margin_invalid_option(right_margin):
    with pytest.raises(bsqlparseError):
        bsqlparse.format('foo', right_margin=right_margin)


@pytest.mark.xfail(reason="Needs fixing")
def test_format_right_margin():
    # TODO: Needs better test, only raises exception right now
    bsqlparse.format('foo', right_margin="79")
