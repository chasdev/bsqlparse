...
def test_grouping_identifier_as_invalid():
    # issue8
    p = bsqlparse.parse('foo as select *')[0]
...
def test_grouping_identifier_function():
    p = bsqlparse.parse('foo() as bar')[0]
    assert isinstance(p.tokens[0], sql.Identifier)
    assert isinstance(p.tokens[0].tokens[0], sql.Function)
    p = bsqlparse.parse('foo()||col2 bar')[0]
    assert isinstance(p.tokens[0], sql.Identifier)
...
def test_grouping_identifier_list_case():
    p = bsqlparse.parse('a, case when 1 then 2 else 3 end as b, c')[0]
    assert isinstance(p.tokens[0], sql.IdentifierList)
    p = bsqlparse.parse('(a, case when 1 then 2 else 3 end as b, c)')[0]
    assert isinstance(p.tokens[0].tokens[1], sql.IdentifierList)
...
def test_grouping_identifier_list_other():
    # issue2
    p = bsqlparse.parse("select *, null, 1, 'foo', bar from mytable, x")[0]
...
def test_grouping_identifier_list_with_inline_comments():
    # issue163
    p = bsqlparse.parse('foo /* a comment */, bar')[0]
    assert isinstance(p.tokens[0], sql.IdentifierList)
    assert isinstance(p.tokens[0].tokens[0], sql.Identifier)
...
def test_grouping_alias_case():
    # see issue46
    p = bsqlparse.parse('CASE WHEN 1 THEN 2 ELSE 3 END foo')[0]
...
def test_grouping_subquery_no_parens():
    ...
    p = bsqlparse.parse('CASE WHEN 1 THEN select 2 where foo = 1 end')[0]
    assert len(p.tokens) == 1
...
def test_grouping_idlist_function():
    # see issue10 too
    p = bsqlparse.parse('foo(1) x, bar')[0]
    assert isinstance(p.tokens[0], sql.IdentifierList)
...
def test_grouping_function():
    p = bsqlparse.parse('foo()')[0]
    assert isinstance(p.tokens[0], sql.Function)
    p = bsqlparse.parse('foo(null, bar)')[0]
    assert isinstance(p.tokens[0], sql.Function)
...
def test_grouping_varchar():
    p = bsqlparse.parse('"text" Varchar(50) NOT NULL')[0]
    assert isinstance(p.tokens[2], sql.Function)


def test_statement_get_type():
    def f(sql):
        return bsqlparse.parse(sql)[0]
...
def test_identifier_consumes_ordering():
    # issue89
    p = bsqlparse.parse('select * from foo order by c1 desc, c2, c3')[0]
...
def test_comparison_with_functions():
    # issue230
    p = bsqlparse.parse('foo = DATE(bar.baz)')[0]
    assert len(p.tokens) == 1
    assert isinstance(p.tokens[0], sql.Comparison)
    assert len(p.tokens[0].tokens) == 5
    assert p.tokens[0].left.value == 'foo'
    ...
    p = bsqlparse.parse('DATE(foo.bar) = DATE(bar.baz)')[0]
    assert len(p.tokens) == 1
    assert isinstance(p.tokens[0], sql.Comparison)
    assert len(p.tokens[0].tokens) == 5
    ...
    p = bsqlparse.parse('DATE(foo.bar) = bar.baz')[0]
    assert len(p.tokens) == 1
    assert isinstance(p.tokens[0], sql.Comparison)
    assert len(p.tokens[0].tokens) == 5
    assert p.tokens[0].left.value == 'DATE(foo.bar)'
...
def test_forloops(start):
    p = bsqlparse.parse('{0} foo in bar LOOP foobar END LOOP'.format(start))[0]
    assert (len(p.tokens)) == 1
    assert isinstance(p.tokens[0], sql.For)


def test_nested_for():
    p = bsqlparse.parse('FOR foo LOOP FOR bar LOOP END LOOP END LOOP')[0]
    assert len(p.tokens) == 1
    for1 = p.tokens[0]
...
def test_nested_begin():
    p = bsqlparse.parse('BEGIN foo BEGIN bar END END')[0]
    assert len(p.tokens) == 1
    outer = p.tokens[0]
...
def test_aliased_column_without_as():
    p = bsqlparse.parse('foo bar')[0].tokens
    assert len(p) == 1
    assert p[0].get_real_name() == 'foo'
    assert p[0].get_alias() == 'bar'

    p = bsqlparse.parse('foo.bar baz')[0].tokens[0]
...
def test_qualified_function():
    p = bsqlparse.parse('foo()')[0].tokens[0]
    assert p.get_parent_name() is None
    assert p.get_real_name() == 'foo'

    p = bsqlparse.parse('foo.bar()')[0].tokens[0]
    assert p.get_parent_name() == 'foo'
    assert p.get_real_name() == 'bar'
...
def test_aliased_literal_without_as():
    p = bsqlparse.parse('1 foo')[0].tokens
    assert len(p) == 1
