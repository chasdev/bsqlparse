...
def validate_options(options):
    ...
    if kwcase not in [None, 'upper', 'lower', 'capitalize']:
        raise bsqlparseError('Invalid value for keyword_case: '
                            '{0!r}'.format(kwcase))

    idcase = options.get('identifier_case')
    if idcase not in [None, 'upper', 'lower', 'capitalize']:
        raise bsqlparseError('Invalid value for identifier_case: '
                            '{0!r}'.format(idcase))

    ofrmt = options.get('output_format')
    if ofrmt not in [None, 'sql', 'python', 'php']:
        raise bsqlparseError('Unknown output format: '
                            '{0!r}'.format(ofrmt))

    strip_comments = options.get('strip_comments', False)
    if strip_comments not in [True, False]:
        raise bsqlparseError('Invalid value for strip_comments: '
    ...
    if space_around_operators not in [True, False]:
        raise bsqlparseError('Invalid value for use_space_around_operators: '
                            '{0!r}'.format(space_around_operators))

    strip_ws = options.get('strip_whitespace', False)
    if strip_ws not in [True, False]:
        raise bsqlparseError('Invalid value for strip_whitespace: '
                            '{0!r}'.format(strip_ws))
    ...
    if truncate_strings is not None:
        try:
            ...
            raise bsqlparseError('Invalid value for truncate_strings: '
    ...
    if reindent not in [True, False]:
        raise bsqlparseError('Invalid value for reindent: '
                            '{0!r}'.format(reindent))
    elif reindent:
    ...
    if reindent_aligned not in [True, False]:
        raise bsqlparseError('Invalid value for reindent_aligned: '
                            '{0!r}'.format(reindent))
    ...
    if indent_tabs not in [True, False]:
        raise bsqlparseError('Invalid value for indent_tabs: '
    ...
    try:
        ...
        raise bsqlparseError('indent_width requires an integer')
    if indent_width < 1:
        raise bsqlparseError('indent_width requires a positive integer')
    ...
    try:
        ...
        raise bsqlparseError('wrap_after requires an integer')
    if wrap_after < 0:
        raise bsqlparseError('wrap_after requires a positive integer')
    options['wrap_after'] = wrap_after

    comma_first = options.get('comma_first', False)
    if comma_first not in [True, False]:
        raise bsqlparseError('comma_first requires a boolean value')
    ...
    if right_margin is not None:
        try:
            ...
            raise bsqlparseError('right_margin requires an integer')
        if right_margin < 10:
            raise bsqlparseError('right_margin requires an integer > 10')
...
def build_filter_stack(stack, options):
    ...
    Args:
      stack: :class:`~bsqlparse.filters.FilterStack` instance
